	...
	public static void main(String[] args){
        ...
        while(!testGrillePleine && blocage!=2){ //tant que la grille n'est pas remplie et que c'est toujours possible de jouer... 
			...
			while(!tour && blocage!=2){ //tant que le jeu n'est pas bloqué et que le tour n'est pas terminé, on laisse le joueur x jouer
				...
				if(tourJoueur==2){ //tour de l'ia
					int xIA=(int)(Math.random()*8); //on génère les valeurs aléatoires pour l'ia
					int yIA=(int)(Math.random()*8);

				
					

					if(grille.getXY(yIA,xIA).getJoueur()==0 && grille.testPosePion(xIA,yIA,tourJoueur)){ //on insère le pion correspondant ainsi que tous ceux qui doivent être remplacés
						try{Thread.sleep(1000);}catch(Exception e){}
						tour=true;
						grille.getXY(yIA,xIA).setJoueur(2);
						...
						tourJoueur=1; //on change de joueur
						
					}
					
				}
				else if(tourJoueur==1){ //tour du joueur
